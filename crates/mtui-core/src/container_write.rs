//! Form schemas for the Container nav group (`/container`, `/app`).
    ...
    sections: &[
        FormSection {
            ...
            fields: &[
                ...
                f!("tmpdir", "Tmpdir", FieldKind::Text),
    ...
    sections: &[
        FormSection {
            ...
            fields: &[
                ...
                f!("layer-dir", "Layer Dir", FieldKind::Text),
                f!("envlist", "Envlist", LOOKUP_ENV_LIST),
                f!("mountlists", "Mountlists", LOOKUP_MOUNT_LISTS),
                ...
                f!("workdir", "Workdir", FieldKind::Text),
...
mod tests {
    ...
    fn assert_lookup(schema: &FormSchema, key: &str, resource_id: &'static str, multiple: bool) {
        match schema.field(key).map(|field| field.kind) {
            Some(FieldKind::Lookup {
                ...
                assert_eq!(multi, multiple);
            }
            other => panic!("{key} lookup, got {other:?}"),
        }
    }
    ...
    fn config_password_is_secret() {
        ...
        assert!(CONTAINER_CONFIG_FORM.create_sections.is_empty());
        assert_eq!(
    ...
    fn container_lookups_and_create() {
        ...
        assert_lookup(&CONTAINER_FORM, "mountlists", "container-mounts", true);
        assert_eq!(
